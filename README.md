...
- **No database migrations run yet.** Part 1 has no persistence — `MovieDto` is a plain DTO, nothing is written to MySQL, and `backend/migrations/` is empty. `docker compose up -d --build` does not run `doctrine:migrations:migrate` anywhere (the backend entrypoint only scaffolds Symfony and starts the server). This will matter starting in Part 2, once real entities/persistence are introduced — migrations will need to be generated and either run automatically on startup or documented as a manual step (`docker compose exec backend php bin/console doctrine:migrations:migrate`).
...
Not yet implemented. See [part2.md](part2.md) for the spec (multiple named favorite lists, persisted via a new `Movie` entity/repository, list/favorite CRUD endpoints). This section will be filled in the same way as Part 1 once that work starts.
